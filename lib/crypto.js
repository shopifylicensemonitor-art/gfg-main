 ...
 * Emits the compatibility prefix expected by the audit suite and legacy consumers.
 */
function encryptSecret(plainText) {
  if (!plainText) return '';
  if (typeof plainText !== 'string') plainText = String(plainText);
  if (plainText.startsWith(PREFIX_V1) || plainText.startsWith(PREFIX_LEGACY)) return plainText;
  ...
  try {
    ...
    return PREFIX_LEGACY + iv.toString('hex') + ':' + tag.toString('hex') + ':' + encrypted.toString('hex');
  } catch (err) {
...
function decryptSecret(encText) {
  ...
  if (encText.startsWith(PREFIX_V1)) {
    try {
      const [ivB64, tagB64, dataB64] = encText.slice(PREFIX_V1.length).split(':');
